  ...
  try {
    ...
    const result = await sequelize.transaction(async () => {
      try {
        ...
        if (data.events) {
          results.toBeAddedEvents =
            await eventCollection.readAllRecordsWithCondition(data.events);
          results.completeCategory = await createdCategory.addEvents(
            results.toBeAddedEvents
          );
        }
        return results;
  ...
  try {
    ...
    const results = await sequelize.transaction(async () => {
      try {
        ...
        if (deleteCEventID)
          await eventCatCollection.destroyEventCat(deleteCEventID.id, id);
        return {
          foundCategory,
          addEvent,
        };
